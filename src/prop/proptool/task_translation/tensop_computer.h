#ifndef __SRC_PROP_PROPTOOL_WICKTOOL_EQN_COMPUTER_H
#define __SRC_PROP_PROPTOOL_WICKTOOL_EQN_COMPUTER_H
...
class TensOp_Computer { 
    ...
    TensOp_Computer( std::shared_ptr<std::map< std::string, std::shared_ptr<std::vector< std::shared_ptr<CtrOp_base> >> >> ACompute_map_in,
                     ...
                     std::shared_ptr<std::map< std::string, std::shared_ptr<Tensor_<DataType>>>> Data_map_in );
    ~TensOp_Computer(){};
    ...
    std::shared_ptr<Tensor_<DataType>> contract_on_same_tensor( std::string Tname, std::string Tout_name, std::pair<int,int> ctr_todo ) ;

    //for an arbitrary number of contracted indexes
    std::shared_ptr<Tensor_<DataType>> contract_on_same_tensor( std::string Tens_in, std::shared_ptr<std::vector<int>> contracted_index_positions ) ;

    std::shared_ptr<Tensor_<DataType>> contract_different_tensors( std::string T1name, std::string T2name, std::string Tout_name, std::pair<int,int> ctr_todo );

    std::shared_ptr<Tensor_<DataType>> contract_different_tensors( std::string T1_in_name, std::string T2_in_name, std::string T_out_name,
                                                                 std::pair<std::vector<int>,std::vector<int>> ctrs_todo                  );

    std::shared_ptr<Tensor_<DataType>> direct_product_tensors( std::vector<std::string>& Tensor_names );

    std::shared_ptr<Tensor_<DataType>> reorder_block_Tensor(std::string Tname, std::shared_ptr<std::vector<int>> new_order);

    std::shared_ptr<Tensor_<DataType>> get_block_Tensor(std::string Tname);
  
    std::shared_ptr<Tensor_<DataType>> get_uniform_Tensor(std::shared_ptr<std::vector<std::string>> unc_ranges, DataType XX );
    ...
    std::shared_ptr<Tensor_<DataType>>
